...
import at.alladin.rmbt.android.main.SimpleTestResultFragment;
...
public class SimpleTestFragment extends Fragment implements ServiceConnection
{
    ...
    private View createView(final View view, final LayoutInflater inflater, final Bundle savedInstanceState) {
        ...
        if (testView != null) {
            final View statusView = view.findViewById(R.id.test_view_status_button_text);
    ...
    private final Runnable updateTask = new Runnable()
    {
        ...
        public void run()
        {
            ...
            if (rmbtService != null) {
                if (rmbtService.isCompleted() && rmbtService.getTestUuid() != null && !rmbtService.isConnectionError()) {
    ...
    private void switchToResult()
    {
        ...
        if (intermediateResult.status != TestStatus.ERROR) {
            ((RMBTMainActivity) getActivity()).showStartScreenAfterTestWithoutSwitchingToResults(testUuid, rmbtService.getIntermediateResult(intermediateResult));
        }
    }
