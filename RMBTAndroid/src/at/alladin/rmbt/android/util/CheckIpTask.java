...
public class CheckIpTask extends AsyncTask<Void, Void, JSONArray>
{
    ...
    protected JSONArray doInBackground(final Void... params)
    {
        ...
        try {
            ...
            Log.d(DEBUG_TAG, "checking IPv4: " + addr.getAddress());
        	s.connect(addr, 5000);
        ...
        try { 
            ...
            Log.d(DEBUG_TAG, "checking IPv6: " + addr.getAddress());
        	s.connect(addr, 5000);
        ...
        if (privateIpv4 != null) {
            ...
            Log.d(DEBUG_TAG, "IPv4 response: " + response);
        ...
        if (privateIpv6 != null) {
            ...
            Log.d(DEBUG_TAG, "IPv6 response: " + response);
        	if (response != null && response.length() >= 1) {
        		newsList.put(response.opt(0));
        	}
        }
    ...
    protected void onPostExecute(final JSONArray newsList)
    {
    	...
    	try {
        	Log.d(DEBUG_TAG, "IpTask: " + newsList);
            String ipv = IpResponse.IpVersion.IP_VERSION_4.toString();
                ...
                for (int i = 0; i < newsList.length(); i++) {
                    if (!isCancelled() && !Thread.interrupted()) {
                        try
                        {
                            ...
                            if (newsItem.has("version"))
                            {
                            	ipv = newsItem.getString("version");
                            	
                                if (IpResponse.IpVersion.IP_VERSION_6.toString().equals(ipv)) {
                                	try {
