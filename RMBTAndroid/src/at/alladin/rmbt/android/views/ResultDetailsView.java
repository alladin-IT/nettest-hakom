...
public class ResultDetailsView extends LinearLayout implements EndTaskListener<JSONArray> {
        ...
        if (testResultDetail != null && testResultDetail.length() > 0 && (testResultDetailTask==null || !testResultDetailTask.hasError()))
        {
            ...
            try
            {
                ...
                for (int i = 0; i < testResultDetail.length(); i++)
                {
                    ...
                    final Object o = singleItem.opt("title");
                    
                    if (o instanceof JSONObject) {
                    	final Iterator<String> keys = ((JSONObject) o).keys();
                    	final JSONObject valueJson = singleItem.optJSONObject("value");
                    	if (valueJson != null) {
	                    	while(keys.hasNext()) {
	                    		viewItem = new HashMap<String, String>();
	                    		final String key = keys.next();
	                    		viewItem.put("name", ((JSONObject) o).optString(key, ""));
	                    		viewItem.put("value", valueJson.optString(key, ""));
