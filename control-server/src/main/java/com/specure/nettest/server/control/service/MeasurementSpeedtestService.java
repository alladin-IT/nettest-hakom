...
import com.specure.nettest.server.control.exception.measurement.MeasurementInvalidException;
import com.specure.nettest.server.control.service.ClassificationService.ClassificationType;
import com.specure.nettest.server.control.service.GeoLocationService.LocationGraph;
...
import com.specure.nettest.shared.model.Settings.SpeedtestDetailGroup;
import com.specure.nettest.shared.model.Settings.SpeedtestDetailGroup.SpeedtestDetailGroupEntry;
import com.specure.nettest.shared.model.Settings.SpeedtestDetailGroup.SpeedtestDetailGroupEntry.FormatEnum;
import com.specure.nettest.shared.model.response.SpeedtestDetailGroupResultResponse;
import com.specure.nettest.shared.model.response.SpeedtestDetailResultResponse;
import com.specure.nettest.shared.model.response.SpeedtestDetailResultResponse.SpeedtestDetailItem;
import com.specure.nettest.shared.model.response.SpeedtestDetailResultResponse.SpeedtestTimeDetailItem;
import com.specure.nettest.shared.model.response.SpeedtestResultResponse;
import com.specure.nettest.shared.model.response.SpeedtestDetailGroupResultResponse.SpeedtestDetailResponseGroup;
import com.specure.nettest.shared.model.response.SpeedtestResultResponse.ClassifiedResultItem;
import com.specure.nettest.shared.model.response.SpeedtestResultResponse.ResultItem;
...
public class MeasurementSpeedtestService {
	...
	private ResultItem generateResultItem(final String titleKey, final String value, final Locale locale) {
		return new ResultItem(getLocalizedMessage(titleKey, locale), value);
	}
	...
	private ClassifiedResultItem generateClassifiedResultItem(final String titleKey, final String value, final int classification, final Locale locale) {
		return new ClassifiedResultItem(getLocalizedMessage(titleKey, locale), value, classification);
	}
	...
	public SpeedtestResultResponse getSpeedtestMainResult(final Locale locale, final Measurement measurement, final HttpServletRequest httpServletRequest) {
        ...
        classifiedResultList.add(generateClassifiedResultItem("RESULT_DOWNLOAD", downloadString, 
        		classificationService.classify(ClassificationType.DOWNLOAD, speedtestInfo.getSpeedDownload()), locale));
        ...
        classifiedResultList.add(generateClassifiedResultItem("RESULT_UPLOAD", uploadString, 
        		classificationService.classify(ClassificationType.UPLOAD, speedtestInfo.getSpeedUpload()), locale));
       
        final String pingString = String.format("%s %s", format.format(speedtestInfo.getPingMedian() / 1000000d),
        		getLocalizedMessage("RESULT_PING_UNIT", locale));
        classifiedResultList.add(generateClassifiedResultItem("RESULT_PING", pingString, 
        		classificationService.classify(ClassificationType.PING, speedtestInfo.getPingMedian()), locale));
        ...
        if (measurement.getSignalStrength() != null || measurement.getLteRsrp() != null) {
        	if (measurement.getLteRsrp() == null) {
                ...
                classifiedResultList.add(generateClassifiedResultItem("RESULT_SIGNAL", signalString, 
                		classificationService.classify(classificationType, measurement.getSignalStrength()), locale));
        	}
        	else {
        		...
        		classifiedResultList.add(generateClassifiedResultItem("RESULT_SIGNAL_RSRP", signalString, 
                		classificationService.classify(ClassificationType.SIGNAL_RSRP, measurement.getLteRsrp()), locale));
        	}
        }
        ...
        response.setShareSubject(messageSource.getMessage("RESULT_SHARE_SUBJECT", new Object[] {timeString}, locale));
...
//                        }                        
	...
	public SpeedtestDetailGroupResultResponse getSpeedtestDetailGroupResult(final Locale locale, final Measurement measurement, boolean includeKeys) {
		...
		//grab the settings and put them into a JsonObject
		final JsonObject settingsJson = databaseGson.fromJson(databaseGson.toJson(settings, Settings.class), JsonObject.class);
		final JsonArray groupedResultsJson = JsonGroupResultsHelper.groupJsonResult(databaseGson.toJson(measurement, Measurement.class), settingsJson.getAsJsonArray("speedtestDetailGroups")).getAsJsonArray("groups");
		
		//Fill in the corresponding Response with translated and formatted values
		final SpeedtestDetailGroupResultResponse response = new SpeedtestDetailGroupResultResponse();
		response.setSpeedtestDetailGroups(new ArrayList<>());
		...
		//groupedResults.getAsJsonArray("groups");
		for(int i = 0; i < groupedResultsJson.size(); i++){
			groups.add(databaseGson.fromJson(groupedResultsJson.get(i), SpeedtestDetailGroup.class));
		}
		...
		for (final SpeedtestDetailGroup sdg : groups) {
			...
			responseGroup.setTitle(getLocalizedMessage("key_" + sdg.getKey(), locale));
			...
			for (SpeedtestDetailGroupEntry entry : sdg.getValues()) {
				...
				item.setTitle(getLocalizedMessage(entry.getTranslationKey(), locale));
				...
				if(key.endsWith("network_type")){
					...
					item.setTitle(getLocalizedMessage("key_timezone", locale));
					item.setValue(String.format("UTC%sh", tzFormat.format(offset)));
					
				}else if(key.endsWith("interface_total_bytes_download") || key.endsWith("interface_total_bytes_upload") || 
						key.endsWith("total_bytes_download") || key.endsWith("total_bytes_upload")){
					
					// TODO: this needs way better logic! -> support for computed values
					
					boolean foundPreviousEntry = false;
					
					for (SpeedtestDetailItem it : responseGroup.getEntries()) {
						if(it.getTitle().equals(getLocalizedMessage(entry.getTranslationKey(), locale))){
							foundPreviousEntry = true;
							
							final NumberFormat nf = NumberFormat.getInstance(locale);
							try {
								final Number numberOld = nf.parse(it.getValue().split(" ")[0]);
								final Number numberNew = nf.parse(formatResultValueString(val, formatEnum, format));
								final double n = numberOld.doubleValue() + numberNew.doubleValue();
								
								val = format.format(n);
								val += " " + getLocalizedMessage(unit, locale);
					...
					val = this.formatResultValueString(val, formatEnum, format);
					val += " " + getLocalizedMessage(unit, locale);
					...
					val = "NONE".equals(entry.getValue()) ? 
							getLocalizedMessage("key_encryption_false", locale) : 
								getLocalizedMessage("key_encryption_true", locale);
			...
			if (sdg.getKey().equals("device_information_group")) {
				...
				if (locations != null && locations.size() > 0) {
					...
					item.setTitle(getLocalizedMessage("key_location", locale));
					
					try{
						item.setValue(getGeoLocation(settings, locale, locations.get(0)));
					} catch(JSONException ex) {
						logger.error("Exception occured during geolocation parse of grouped result", ex);
					}
					
					responseGroup.getEntries().add(item);
				}
			}
			response.getSpeedtestDetailGroups().add(responseGroup);
		}
		return response;
	}
        ...
        if (location != null && latitude != null && longitude != null && accuracy != null) {
            if (accuracy < settings.getRmbt().getGeoAccuracyDetailLimit()) {
                ...
                if (provider != null) {
                    ...
                    geoString.append(messageSource.getMessage(key, null, locale));
                    geoString.append(", ");
                }
...
//            }
    ...
    public String getRoamingType(final MessageSource labels, final int roamingType, final Locale locale)
    {
        ...
        {
        case 0:
            roamingValue = labels.getMessage("value_roaming_none", null, locale);
            break;
        case 1:
            roamingValue = labels.getMessage("value_roaming_national", null, locale);
            break;
        case 2:
            roamingValue = labels.getMessage("value_roaming_international", null, locale);
            break;
        default:
            roamingValue = "?";
            break;
