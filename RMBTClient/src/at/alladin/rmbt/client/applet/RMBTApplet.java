                ...
                try
                {
                    ...
                    if (runRMBT)
                    {
                        ...
                        */
                        ...
                        final MeasurementRequest request = new MeasurementRequest();
                        request.setNdt(ndtActivated);
                        request.setPlatform("Applet");

                        client = RMBTClient.getInstance(getParameter("host"), getParameter("path"),
                                port, encryption, geoInfo, uuid, ClientType.DESKTOP, Config.RMBT_CLIENT_NAME,
                                Config.RMBT_VERSION_NUMBER, null, request);
                    ...
                    if (runNdt && (client.getStatus() == TestStatus.QOS_END || client.getStatus() == TestStatus.SPEEDTEST_END))
                    {
                        ...
                        ndtRunner.runNDT(NdtTests.NETWORK_WIRED, ndtRunner.new UiServices()
                        {
                            ...
                            public void sendResults()
                            {
                                ...
                                final ControlServerConnection csc = new ControlServerConnection(encryption, getParameter("host"), getParameter("path"), port);
